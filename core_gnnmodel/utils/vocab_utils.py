...
class VocabModel(object):
    def __init__(self, data_set, config):
        ...
        allWords, allEdges = collect_vocabs(data_set)
        print('Number of src words: {}'.format(len(allWords)))
        print('Number of edges: {}'.format(len(allEdges)))
        ...
        self.edge_vocab.build_vocab(allEdges)
...
def collect_vocabs(all_instances):
    ...
    for sent1 in all_instances:
        for edge in sent1.graph['edges']:
            all_edge_types.update([edge[0]])
        for node in sent1.graph['nodes']:
            all_src_words.update(node['content'].split(' '))
    return (all_src_words, all_edge_types)