...
class Graph(object):
    def __init__(self, instance, config, isLower=False):
        code_graph = instance['multi_graph']
        self.config = config
        filter_code_nodes, edges, max_token_in_node = self.build_code_graph(code_graph, isLower)
        ...
        self.max_token_in_node = max_token_in_node
    ...
    def build_code_graph(self, code_graph, isLower):
        ...
        for edge in edges:
            if edge[0] in ['IS_AST_PARENT', 'FLOWS_TO', 'DEF', 'REACHES', 'USE', 'CONTROLS','CALL_GRAPH']:
